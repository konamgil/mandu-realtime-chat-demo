...
test.describe("e2e: login → chat → send → UI reflect (+ SSE reconnect catch-up)", () => {
  test("demo login, send one message, then reconnect and catch up missed message", async ({ page, request, baseURL }) => {
    ...
    if (await page.getByRole("heading", { name: "🔐 로그인" }).isVisible().catch(() => false)) {
      await page.evaluate(() => {
        window.localStorage.setItem(
          "mandu-chat-demo.session",
          JSON.stringify({
    ...
    await expect(page.getByPlaceholder("메시지를 입력하세요")).toBeVisible({ timeout: 30_000 });
    ...
    await expect(page.getByText(/상태:/)).toContainText("연결중", { timeout: 15_000 });
